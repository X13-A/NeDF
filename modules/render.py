...
def march_rays(model, dataset, rays, position, num_samples=50, threshold=0.01, max_distance=10.0, device="cpu"):
    num_rays = rays.shape[0]
    total_distances = torch.zeros(num_rays, device=device)  # Store total distance per ray

    # Initial positions for each ray (starts at camera position)
    current_positions = position.unsqueeze(0).expand(num_rays, -1).to(device)  # (10000, 3)

    for _ in range(num_samples):
        # Query the model for distance at the current position
        # distances = model(current_positions).squeeze(-1)
        distances = estimate_distances(current_positions, dataset, device, False)
        # Check which rays are close enough to stop marching
        mask = distances > threshold

        # Stop marching for rays that have already reached a surface
        if not mask.any():
            break

        # Move forward along the ray direction, scaled by predicted distance
        step = rays[mask] * distances[mask].unsqueeze(-1)
        current_positions[mask] += step

        # Accumulate total traveled distance
        total_distances[mask] += distances[mask]

        # Stop if the total traveled distance exceeds the max allowed distance
        total_distances[total_distances > max_distance] = max_distance
